...
Client `SetPixelFormat` **is** honored for true-color screens: `vncSetPixelFormat` stores the requested format via `setPixelFormat`, `fbSyncTasks`→`updateColorTable` applies it to `fbPixFormat`, and `copyNativeRowToWire`/`copyNativeTileToCPIXEL` convert native Mac pixels into it (`packWire`/`emitWire`, with a `wireIs888BE` fast path). This matters because RealVNC (and others) default to requesting a *reduced* pixel (8/16-bit) — ignoring it kept sending 32-bit pixels and desynced the stream ("bad rectangle … exceeds signed 16-bit values" on the client). Only the client's byte-count/format is honored per-pixel; a genuine **indexed/colormap** request (`trueColor==0`) on a true-color screen is still unsupported and fails cleanly with `VNC_ERROR` (the "incrementally full honor" follow-up: quantize 32-bit → 8-bit palette).

**Encoder subrect contract:** `VNCEncoder::isNewSubrect()` decides when to emit a rectangle header, keyed on `tile_x/tile_y`. Tile encoders (ZRLE/TRLE/Hextile) advance those as they walk tiles, so the header fires once (or once per ZRLE 192-subrect). The **Raw** encoder walks rows via its own `line` counter, so it must keep `tile_y` in step (`VNCEncodeRaw::getChunk` sets `tile_y = line`) — otherwise `isNewSubrect()` stays true and the rect header is re-emitted before every row, drifting the image and desyncing the client. Any new non-tile encoder must do the same.

Known remaining follow-ups: true-color **ZRLE** likely still mis-frames wide/short subrects (Raw is the reliable true-color path); the cursor pseudo-encoding reads a NULL true-color table on native true-color screens (garbage cursor colors, not a crash); and the `VNC_FB_16BIT/32BIT` fixed-depth build without a fixed `VNC_FB_WIDTH` won't compile (`VNCFrameBuffer.h` `VNC_FB_PIX_PER_BYTE` undefined).
